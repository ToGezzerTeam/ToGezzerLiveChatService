...
describe('WsJwtAuthService', () => {
  ...
  it('authenticates with token from websocket auth payload', () => {
    const token = jwtService.sign({
      sub: 'uuid-2',
      id: 2,
      uuid: 'uuid-2',
    ...
    expect(payload.sub).toBe('uuid-2');
    expect(payload.uuid).toBe('uuid-2');
    expect(socket.data.user?.username).toBe('user2');
  });
